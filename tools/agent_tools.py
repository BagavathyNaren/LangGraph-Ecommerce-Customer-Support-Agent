...
def register_customer(name: str, email: str, config: RunnableConfig, phone_number: str = None) -> str:
    """Register a new customer account. Use when a customer wants to register, or when they want to place an order but don't have an account yet. Requires their full name and email. Phone number is optional but recommended for SMS notifications on GCP."""
    ...
    result = register_new_customer(name, email, phone_number)
    return str(result)
