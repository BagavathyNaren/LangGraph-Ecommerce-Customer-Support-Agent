...
def is_country_supported(country: str) -> bool:
    ...
    supported = {
        "uae", "united arab emirates", "emirates", "ae",
        "japan", "jp",
...
def update_order_from_webhook(order_id: str, new_status: str, carrier: str = None) -> dict:
    ...
    try:
        ...
        logger.error("DB error in update_order_from_webhook", extra={"event": "db_error", "error": str(e), "order_id": order_id})
        return {"error": USER_FRIENDLY_DB_ERROR}
        ...
        with get_connection() as conn:
            with conn.cursor() as cur:
                ...
                if not re.match(r"^[\w\.-]+@[\w\.-]+\.\w+$", email) or len(email) < 6:
                    return {"success": False, "message": "Invalid email address format. Please provide a fully valid email address."}
...
def extract_price_from_text(text: str, default_currency: str = "INR") -> str:
    ...
    cleaned = re.sub(r'\b\d+\s*(?:MP|GB|RAM|Hz|W|mAh|cm|inch|inches|fps|GB|TB|Gen\s*\d|Core\s*i\d|Ryzen\s*\d)\b', '', text, flags=re.IGNORECASE)
    ...
    for m in re.finditer(r'\b(?:price|cost|rate|sale|buy|at)\s*[:=-]?\s*([1-9]\d{1,}(?:,\d{3})*(?:\.\d{2})?)\b', cleaned, flags=re.IGNORECASE):
        if not is_subscription_or_protection(m.start(), m.end(), cleaned):
...
def clean_title_for_search(title: str) -> str:
    ...
    cleaned = re.sub(r'^(?:Amazon\.(?:com|in|co\.jp|cn)|Amazon|Flipkart|Croma|亚马逊海外购|亚马逊)\s*[:|：|-]\s*', '', cleaned, flags=re.IGNORECASE)
    # Remove common suffix patterns (e.g. " : Everything Else - Amazon.com", " - Croma")
    cleaned = re.sub(r'\s*[:|：|-]\s*(?:Amazon\.(?:com|in|co\.jp|cn)|Amazon|Flipkart|Croma|亚马逊海外购|亚马逊|Video Games|Electronics|Everything Else|Home Audio|Mobile Phones|Smartphones).*$', '', cleaned, flags=re.IGNORECASE)
...
def fetch_retailer_data(platform: str, query: str, country: str = 'India') -> dict:
    ...
    if "usa" in country_clean or "united states" in country_clean or country_clean == "us":
        ...
        hl = "ja"
        currency = "JPY"
    elif "uk" in country_clean or "united kingdom" in country_clean or "britain" in country_clean or country_clean == "gb":
        amazon_domain = "amazon.co.uk"
        gl = "gb"
        hl = "en"
        currency = "GBP"
    elif "uae" in country_clean or "united arab emirates" in country_clean or "emirates" in country_clean or country_clean == "ae":
    ...
    if platform in ["flipkart", "croma"] and not is_india:
        return {"success": False, "message": f"{platform_name} is only available in India. Please use Amazon for {country}."}
    
    # Generic storefront titles that indicate a useless result (not a real product listing)
    GENERIC_TITLES = {
        "amazon", "amazon.co.jp", "amazon.in", "amazon.com", "amazon.co.uk", "amazon.ae",
        "amazon global store", "amazon international",
        "flipkart", "croma", "flipkart.com", "croma.com"
    }
    ...
    def is_valid_price(price: str) -> bool:
        ...
        if p_clean in ["unknown", "not listed", "not_listed", "n/a", "tbd", "none", "null", "not available", "not_available"]:
            return False
    ...
    if serper_key:
        try:
            ...
            if not shopping_results:
                ...
                res_json = r.json()
                shopping_results = [
                    item for item in res_json.get("shopping", [])
...
def place_new_order(customer_name: str, item: str, customer_email: str = None, product_id: str = None, price: str = None, currency: str = None, source_website: str = None, category: str = 'General') -> dict:
        ...
        with get_connection() as conn:
            with conn.cursor() as cur:
                ...
                try:
                    ...
                    logger.info("Order confirmation successfully validated against strict schema", extra={
                        "event": "order_validated",
                        "order_id": order_id
                    })
                except Exception as schema_err:
                    logger.error(f"Order validation schema check failed: {schema_err}", extra={
