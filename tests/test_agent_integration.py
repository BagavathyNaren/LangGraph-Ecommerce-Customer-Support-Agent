...
def test_graph_compilation(mock_db_connection):
    ...
    """
    with patch("langgraph.checkpoint.postgres.PostgresSaver.setup") as mock_setup:
...
def test_scenario_a_flawless_flow(mock_db_connection):
    ...
    try:
        ...
        state = {"messages": [HumanMessage(content="My name is Shin. I am in the UK. I want to buy a Sony BRAVIA 4K TV")]}
        ...
        has_search = any(
            ...
            for m in res1["messages"] if isinstance(m, AIMessage) and getattr(m, "tool_calls", None)
        ...
        has_order = any(
            ...
            for m in res2["messages"] if isinstance(m, AIMessage) and getattr(m, "tool_calls", None)
            for tc in m.tool_calls
        )
...
def test_scenario_b_unregistered_halt(mock_db_connection):
    ...
    def mock_agent_llm_invoke(messages, *args, **kwargs):
        ...
        if num_msgs == 2:  # System + User intro
            ...
            return AIMessage(content="Here are the Xbox options for Japan:\n\n### Amazon\nXbox Series X\nPrice: ￥45,000")
            
        elif num_msgs == 8:  # User selects option (checkout) -> Ask for email!
            return AIMessage(content="I see you are not registered yet. Could you please provide your email address to complete registration and place your order?")
        ...
        elif num_msgs == 13:  # After parallel register + create tool outputs (System + User1 + AI1 + T1 + AI2 + T2 + AI3 + User2 + AI4 + User3 + AI5 + T5_1 + T5_2)
            return AIMessage(content="Welcome Zara! Your registration is complete, and order ORD99988 has been successfully placed!")
    ...
    try:
        ...
        has_reg = any(
            ...
            for m in res3["messages"] if isinstance(m, AIMessage) and getattr(m, "tool_calls", None)
            for tc in m.tool_calls
        )
